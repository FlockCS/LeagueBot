...
# directly, one item per (discord_id, day). The weekly recap sums the last 7 items
...
#
#   PK: discord_id (String)
#   SK: date       (String, "YYYY-MM-DD")
#   Attrs:
#     name:  Riot display name as of that day
...
def save_daily(date, discord_id, name, hours):
...
def load_week(week_start, discord_id):
    ...
    res = _table.query(
        KeyConditionExpression=Key("discord_id").eq(discord_id) & Key("date").gte(_date_str(week_start)),
    )
    return res.get("Items", [])


def delete_daily_before(cutoff_date, discord_id):
    ...
    res = _table.query(
        KeyConditionExpression=Key("discord_id").eq(discord_id) & Key("date").lt(_date_str(cutoff_date)),
    )
    with _table.batch_writer() as batch:
        for item in res.get("Items", []):
            batch.delete_item(Key={"discord_id": item["discord_id"], "date": item["date"]})
